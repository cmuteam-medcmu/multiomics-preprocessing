...
FASTQ_1=${FASTQ_DIR}/${PATIENT}_WES_1.fp.fastq.gz
FASTQ_2=${FASTQ_DIR}/${PATIENT}_WES_2.fp.fastq.gz
...
# ===== Output Files =====
OUTPUT_BAM=${OUT_DIR}/${PATIENT}_WES.bam
OUTPUT_RECAL_BAM=${OUT_DIR}/${PATIENT}_WES.recal.bam
...
apptainer exec --nv \
  ...
  --knownSites ${KNOWN_SITES_SNP} --knownSites ${KNOWN_SITES_INDEL} \
  --out-bam ${OUTPUT_BAM} \
...
apptainer exec --nv \
  ...
  /common/sif/clara-parabricks/4.5.1-1.sif pbrun \
      ...
      --out-bam ${OUTPUT_RECAL_BAM} \
      --tmp-dir ${OUT_DIR}/tmp

samtools index -@ 20 -b ${OUTPUT_RECAL_BAM}
rm -r ${OUT_DIR}/tmp # Clean up

echo "Normal WES BAM is ready at ${OUTPUT_RECAL_BAM}"
