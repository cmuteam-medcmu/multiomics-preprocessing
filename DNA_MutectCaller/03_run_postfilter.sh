...
check_inputs() {
  ...
  [[ -n "${TUMOR_SAMPLE_ID}" ]] || {
    echo "ERROR: TUMOR_SAMPLE_ID is required."
    exit 1
  }
  [[ -n "${NORMAL_SAMPLE_ID}" ]] || {
    echo "ERROR: NORMAL_SAMPLE_ID is required."
    exit 1
  }
  ...
  *)
  ...
  [[ -f "${UNFILTERED_VCF}" ]] || {
    echo "ERROR: Unfiltered VCF not found: ${UNFILTERED_VCF}"
    exit 1
  }
  [[ -f "${CONTAMINATION_TABLE}" ]] || {
    echo "ERROR: Contamination table not found: ${CONTAMINATION_TABLE}"
    exit 1
  }
  [[ -f "${SEGMENTS_TABLE}" ]] || {
    echo "ERROR: Segments table not found: ${SEGMENTS_TABLE}"
    exit 1
  }
